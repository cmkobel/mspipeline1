...
df_workflow = tryCatch({read_delim(paste0(batch, "/fragpipe/fragpipe_modified.workflow"), delim = "=", comment = "#", col_names = c("name", "value"))})
...
tryCatch({
    ...
    cells$db = list()
    cells$db$n_genes = (df_db_stats %>% filter(name == "n_records_in_db"))$value
    cells$db$n_db_glob = (df_db_stats %>% filter(name == "db_glob_read"))$value %>% str_split(" ") %>% unlist() %>% length()
